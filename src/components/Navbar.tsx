...
export default function Navbar() {
  ...
  const [filteredSuggestions, setFilteredSuggestions] = useState<any[]>([]); // Change the type to any[]
  ...
  const fetchStories = async (query: string) => {
    ...
    try {
      const response = await fetch(
        `http://62.72.46.248:1337/api/stories?filters[title][$containsi]=${query}&populate=cover_image`
      );
      if (!response.ok) {
  ...
  const handleSelectSuggestion = (suggestion: any): void => {
    setSearchQuery(suggestion.title); // Assuming each suggestion is a story object
    setShowSuggestions(false);
  };
          ...
          <nav className="hidden lg:flex items-center justify-end flex-1">
            <ul className="flex items-center gap-6 xl:gap-8">
              ...
              <li className="relative w-full max-w-[16rem] xl:max-w-[20rem]">
                ...
                {loading && (
                  <div className="absolute z-20 mt-2 w-full bg-gray-900 text-white p-2 text-center">
                    Loading...
                  </div>
                )}
                {error && (
